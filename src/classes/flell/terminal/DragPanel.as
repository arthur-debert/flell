package flell.terminal
{
	...
	public class DragPanel extends Panel
	{
	    ...
	    public var dragXOffset : Number;
	    public var dragYOffset : Number;
		...
		public  function resizeHandler(event:MouseEvent):void
		{
			...
			if (upperLeftX <= panelRelX && panelRelX <= lowerLeftX)
			{
			    ...
			    var absPoint : Point = this.globalToLocal(new Point(event.stageX, event.stageY));
			    if(absPoint.y < 30){
			        trace("will drag", event.localY);
			        dragXOffset = event.localX;
			        dragYOffset = event.localY;
			        dragging = true;
		...
		public function doStopDrag(e : Event) : void{
		    removeEventListener(Event.ENTER_FRAME, updateDrag);
		}
		public function updateDrag(evt : Event) : void{
		    x = parent.mouseX - dragXOffset;
		    y = parent.mouseY - dragYOffset;
		}		
	}
}